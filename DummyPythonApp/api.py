...
def conf_database():
    ...
    """
    filename = os.path.join("C:\Patriot\DDServiceAPI\DDService\config\database.json")
...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        mycursor = mydb.cursor()
        mycursor.execute('SELECT * FROM users WHERE user_name = %s AND password = %s', (user_name, password,))
...
def register():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        mycursor = mydb.cursor()
        mycursor.execute('SELECT * FROM users WHERE user_name = %s', (user_name,))
        account = mycursor.fetchone()
        if account:
...
def settings():
    if 'loggedin' in session:
        return render_template('template_settings.html', user_name=session['user_name'])
    return redirect(url_for('login'))
